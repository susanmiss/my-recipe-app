...
const UpdateRecipe = (props) => {
    ...
    const {recipeTitle, recipeContent, id} = state

 
        useEffect(() => {
            axios.get(`${process.env.REACT_APP_API}/${props.match.params.id}`)
            .then(response => {
                const {recipeTitle, recipeContent, id} = response.data;
                setState({...state, recipeTitle, recipeContent, id}) 
            }) 
        ...
        const handleChange = (name) => (event) => {          
            ...
            const handleSubmit = event => {
                ...
                axios
                .put(`${process.env.REACT_APP_API}/${id}`, {recipeTitle,recipeContent})
                .then(response => {
